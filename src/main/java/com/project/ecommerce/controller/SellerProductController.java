...
public class SellerProductController {
    ...
    public ResponseEntity<String> updateProduct(@PathVariable Long productId,@Valid @RequestBody ProductRequest productRequest, @RequestHeader("X-User-Id") Long userId, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            String errorMessage = bindingResult.getFieldErrors().stream()
                    .sorted(Comparator.comparing(FieldError::getField))
                    .map(err -> err.getDefaultMessage())
    ...
    public ResponseEntity<String> deleteProduct(@PathVariable Long productId, @RequestHeader("X-User-Id") Long userId) {
        Long sellerId = userId;
        try {
    ...
    public ResponseEntity<?> getProducts(@RequestHeader("X-User-Id") Long userId) {
        Long sellerId = userId;
        try {
    ...
    public ResponseEntity<?> searchProducts(@RequestParam(required = false) String query, HttpServletRequest request, @RequestHeader("X-User-Id") Long userId) {
